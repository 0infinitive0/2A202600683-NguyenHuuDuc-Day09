...
async def check_routing(state: LegalState) -> dict:
    ...
    needs_tax = any(kw in question_lower for kw in ["tax", "irs", "thuế", "evasion"])
    needs_compliance = any(kw in question_lower for kw in ["compliance", "sec", "regulation", "sox"])
...
async def call_tax_specialist(state: LegalState) -> dict:
    ...
    llm = get_llm()
    agent = create_agent(model=llm, tools=[search_tax_law], system_prompt=tax_prompt)
...
async def call_compliance_specialist(state: LegalState) -> dict:
    ...
    llm = get_llm()
    agent = create_agent(model=llm, tools=[search_compliance_law], system_prompt=compliance_prompt)
    result = await agent.ainvoke({"messages": [{"role": "user", "content": state["question"]}]})
...
async def main():
    ...
    print("  3. Tax, Compliance, and Privacy specialists run IN PARALLEL (LangGraph Send API)")
    ...
    print("\nSaving graph to architecture.png...")
    png_data = graph.get_graph().draw_mermaid_png()
    with open(os.path.join(os.path.dirname(__file__), "architecture.png"), "wb") as f:
        f.write(png_data)
    print("Saved!")
